...
def _openai_tts(text: str, output_path: Path, voice: str = "echo") -> Path:
    import openai
...
def generate_tts(text: str, output_path: Path, voice: str = "echo") -> Path:
    """
    ElevenLabs (wenn ELEVENLABS_API_KEY gesetzt) → OpenAI Fallback.
    """
    words = text.split()
    if len(words) > 155:
        text = " ".join(words[:155]) + "."
